...
The runtime flow now lives on the canonical `PrehistoryRuntimePhase` ladder (`WorldSeeding`, `BiologicalDivergence`, `SocialEmergence`, `WorldReadinessReview`, `FocalSelection`, `SimulationEngineActivePlay`, `GenerationFailure`). Monthly systems always run from the same pipeline, with the orchestration layer pausing and evaluating in `WorldReadinessReview` and freezing while `FocalSelection` holds time before handing the live monthly loop to `SimulationEngineActivePlay`. Legacy `WorldStartupStage` labels now only provide generator diagnostics and UI-friendly subphase labels; the runtime truth for gating ticks has moved entirely into `PrehistoryRuntimePhase`, while startup presentation detail comes from `PrehistoryRuntimeDetailView` rather than the old pass ladder.
...
PR-4 now owns the candidate layer that follows that readiness verdict. `PrehistoryCheckpointEvaluationAdapter` observes the current world, evaluates PR-3 candidate readiness, then invokes `PrehistoryCandidateSelectionEvaluator` to apply the canonical viability gates, map maturity bands from current simulated condition, score only viable starts, and compose the surfaced pool through seed + diversify + fill. `FocalSelection` therefore freezes on top of structured PR-4 candidate summaries rather than a legacy-scored profile shortcut.
Startup remains robust: it is now built around real focal candidates, not invented entries, thanks to explicit checkpoint outcomes. The same monthly pipeline now runs for prehistory and active play, the evaluation checkpoints only read from simulation facts, and `StartupProgressRenderer` owns the console until `PrehistoryRuntimePhase.FocalSelection` hands things to `ChronicleWatchRenderer`.

`FocalSelection` now explicitly freezes advancement while candidates are reviewed; `ChronicleWatchRenderer` keeps time paused, displays the candidate pool and an explicit `Handoff summary`, and resumes the shared pipeline only after the selected end-of-month state is converted into the canonical PR-6 handoff package and `LiveChronicleStartYear` marks the chronicle boundary. That conversion does not advance another month, preserves discoveries-versus-learned truth, maps starts into truthful `Society` / `Polity` control and `Network` / `AnchoredHomeRange` / `TerritorialCore` spatial interpretation, and leaves the `SimulationEngine` paused on entry. A `GenerationFailure` checkpoint leaves the world paused with a clear failure summary, does not start the live chronicle boundary, and remains representable as a world state rather than only as a thrown exception.
...
During `WorldSeeding` and early `BiologicalDivergence` detail views, the orchestration layer can stop the monthly pipeline after the ecological or evolutionary portions that are actually available in that simulated era.
During later `SocialEmergence` detail views, the same monthly pipeline extends through social and polity-bearing systems once those actors exist.
When the runtime enters `WorldReadinessReview`, no extra hidden month is advanced; evaluator logic reads the just-completed end-of-month state.
If the checkpoint resolves to `FocalSelection`, time freezes on that same end-of-month state while the player reviews real simulated candidates.
Only after selection does `SimulationEngineActivePlay` begin, and the handoff uses that exact selected end-of-month state without advancing another month.
