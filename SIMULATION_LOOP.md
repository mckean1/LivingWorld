...
During `BootstrapWorldFrame` and early `PrehistoryRunning` detail views, the orchestration layer can stop the monthly pipeline after the ecological or evolutionary portions that are actually available in that simulated era.
During later `PrehistoryRunning` detail views, the same monthly pipeline extends through social and polity-bearing systems once those actors exist.
When the runtime enters `ReadinessCheckpoint`, no extra hidden month is advanced; evaluator logic reads the just-completed end-of-month state.
...
Only after selection does `ActivePlay` begin, and the handoff uses that exact selected end-of-month state without advancing another month.
