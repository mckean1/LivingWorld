...
Runtime orchestration now runs on the canonical `PrehistoryRuntimePhase` flow (`BootstrapWorldFrame`, `PrehistoryRunning`, `ReadinessCheckpoint`, `FocalSelection`, `ActivePlay`, `GenerationFailure`), so the monthly loop stays the same pipeline from prehistory into active play while checkpoints and candidate selection run in the orchestration layer above the simulation truth. The watch UI can now swap between chronicle and inspection screens without changing the simulation state it observes. Legacy `WorldStartupStage` labels now exist only for generator diagnostics and presentation-friendly subphase names; the runtime truth is the `PrehistoryRuntimePhase` flow plus runtime-owned `PrehistoryRuntimeDetailView`, which keeps monthly ticks shared from prehistory straight through active play without introducing forked pipelines.

Startup presentation now mirrors that runtime truth. `StartupProgressRenderer` owns the console through bootstrap and the shared prehistory pipeline, rendering the canonical phase/subphase text, world-age progress, readiness window status, and deep metrics without writing to the chronicle. When a checkpoint resolves to `FocalSelection`, the panel freezes monthly advancement, `ChronicleWatchRenderer` takes over with the candidate pool plus an explicit `Handoff summary`, and PR-6 builds the canonical active-play handoff package from the exact selected end-of-month state while `LiveChronicleStartYear` marks the chronicle boundary before `ActivePlay` resumes. The conversion preserves inherited summary, knowledge / visibility truth, routes, settlements, neighbors, unresolved shocks, and converted control state without advancing another month, and active play begins paused. If a checkpoint instead reports `GenerationFailure`, the panel stays paused with an honest failure summary instead of inventing a fallback start.
